    ...
    <!-- Main content -->
          ...
          <div class="container" data-aos="fade-up">
            ...
            <article class="article">
              ...
              <div class="hero-img" data-aos="zoom-in">
                <img src="assets/img/blog/blog-post-3.webp" alt="Featured blog image" class="img-fluid" loading="lazy" id = 'cover_img'>
                <div class="meta-overlay">
                  <div class="meta-categories">
                    <a href="#" class="category" id="category">Web Development</a>
              ...
              <div class="article-content" data-aos="fade-up" data-aos-delay="100">
                <div class="content-header">
                  <h1 class="title" id="title">Modern Web Development: Best Practices and Future Trends for 2025</h1>
                  ...
                  <div class="author-info">
                    ...
                    <div class="post-meta">
                      <span class="date" id="date"><i class="bi bi-calendar3"></i> Mar 15, 2025</span>
                      <span class="divider">•</span>
      ...
      <div class="col-lg-4 sidebar">
        ...
        <div class="widgets-container" data-aos="fade-up" data-aos-delay="200">
          ...
          <!-- Categories Widget -->
          <div class="categories-widget widget-item" id="category_items">
          ...
          <!-- Tags Widget -->
          <div class="tags-widget widget-item" id="author_items">
...
<script>
  $(document).ready(function() {
    getData('blog_item', <?= $blog_id;?>);
    loadRecentPost('blog_recent');
    loadFilterItem('category_item');
    loadFilterItem('author_item');


    // Category filter
    $(document).on('click', '#category_items a', function(e) {
      e.preventDefault();
      var category_id = Number($(this).attr('id'));
      getData({
        reqType: 'blog_category_item',
        id: category_id
      });
    });

    //Author filter
    $(document).on('click', '#author_items a', function(e) {
      e.preventDefault();
      var author_id = Number($(this).attr('id'));
      getData({
        reqType: 'blog_author_item',
        id: author_id
      });
    });
  ...
  // formats the date(needs improvement)
  function formatDate(date) {
    const parts = date.split(' ')
    return parts[0]; // yyyy-mm-dd
  }

  // trims the words at the end and adds `...`
  function formatContent(text) {
    let words = text.split(" ");
    let trimmed = words.slice(0, 10).join(" ") + "...";
    return trimmed;
  }

  //Main data function
  function getData(reqType,id) {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (res.statusCode === 200) {
          // let image = ;
          $('#cover_img').attr('src','<?= BLOG_URL?>'+res.data.cover_img);
          $('#category').text(res.data.blog_cat_name);
          $('#title').text(res.data.blog_title);
          $('#info h4').text(res.data.author_name);
        }
  ...
  // loads recent posts
  function loadRecentPost(reqType, limit = 6) {
    $.ajax({
      type: "POST",
      url: "./Apis/load.php",
      data: {
      ...
      success: function(response) {
        ...
        if (res.statusCode === 200) {
          $(res.data).each(function(index, val) {
            htmlData += `
              <div class="post-item">
                <img src="<?= BLOG_THUMB_URL ?>${val.thumbnail_img}" alt="" class="flex-shrink-0" style="width: 80px; margin-right: 15px;">
                <div>
                  <h4><a href="blog-details.html?id=${val.blog_id}">${val.blog_title}</a></h4>
                  <time datetime="${formatDate(val.created_at)}">${formatDate(val.created_at)}</time>
                </div>
              </div><!-- End recent post item-->            
            `;
          });
  ...
  // Loads the category and authors
  function loadFilterItem(reqType) {
    $.ajax({
      type: "POST",
      url: "./Apis/load.php",
      data: {
      ...
      success: function(response) {
        ...
        var htmlData = '';
        if (res.statusCode === 200) {
          if (reqType == 'category_item') {
            $(res.data).each(function(index, val) {
              htmlData += `<li><a href="#" id="${val.blog_cat_id}">${val.blog_cat_name} <span>(${val.blog_count})</span></a></li>`;
            });

            $('#category_items ul').empty();
            $('#category_items ul').html(htmlData);
          }
          if (reqType == 'author') {
            $(res.data).each(function(index, val) {
              htmlData += `<li><a href="#" id="${val.author_id}">${val.author_name}</a></li>`;
            });
            $('#author_items ul').empty();
            $('#author_items ul').html(htmlData);
